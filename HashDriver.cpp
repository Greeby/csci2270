...
int main(int argc, char* argv[]){
    HashTable hash = HashTable(40009);
    int testData[40000];
    float insert[400];
    float search[400];
    ...
    for(int i=0; i<40000; i++)
    {
        ...
        if(i%100 == 0)
        {
            ...
            start = high_resolution_clock::now();
            for(int j = 0; j < 100; j++){
                hash.searchItem(testData[arr[j]]);
            }
            end = high_resolution_clock::now();
            duration<double, std::milli> time_search = end - start;
            search[timeIndex] = (float)time_search.count()/100;
            ///////
            insertCollisions[timeIndex] = hash.getTotalInsertCol();
            searchCollisions[timeIndex] = hash.getTotalSearchCol();
            timeIndex++;
            start = high_resolution_clock::now();
        }
    ...
    ofstream outputFile("insert_search_performance_HashTable_Chaining_dataSetB.csv");
    for(int a = 0; a < 400; a++){
        outputFile << insert[a] << "," << search[a] << "," << insertCollisions[a] << "," << searchCollisions[a] <<endl;
            }
